...
First, we extract embeddings of our preference samples from the model and calculate the differences between helpful and harmful embedding pairs. We then apply singular value decomposition (SVD) to extract the "alignment subspace"—the  directions in the embedding space that most effectively differentiate helpful from harmful responses.

A trick we add here is to remove subspace directions that are already well-represented in the original LLM embedding. This prevents any single direction from dominating the editing process and ensures we only add necessary new directions to the embedding space.

### Step 3: Edit Embeddings During Inference

Finally, during inference when generating a new response, we modify the model's hidden representations by projecting them in the direction of the alignment subspace. This "nudges" the model toward generating more aligned content.
