...
Let's say we have test image that does not follow the training correlations (e.g., landbird over water). In the embedding space, this might be  $x=0.7z_{\texttt{water}}+ 0.3 z_{\texttt{landbird}}$. Our class embeddings might be $c^{\texttt{waterbird}}=0.4z_{\texttt{water}}+0.6z_{\texttt{waterbird}} \text{ and }c^{\texttt{landbird}}=0.4z_{\texttt{land}}+0.6z_{\texttt{landbird}}$. Then, our zero-shot prediction is $x^T c^{\texttt{waterbird}}= 0.1 > x^T c^{\texttt{landbird}}= -0.1$, which gives us waterbird prediction and is incorrect. 
<p align="right">
<img src="https://sprocketlab.github.io/images/blogposts/roboshot/mad-super.gif"  width="200">
</p>
We have seen how in this example, harmful components contained in $x$ caused wrong predictions. Now, let's see how Roboshot improves robustness against these unwanted correlations by reducing harmful components in embeddings and boosting the helpful ones.
